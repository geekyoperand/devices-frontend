...
class Dashboard extends Component {
    ...
    fetchDevices = async () => {
        ...
        this.setState({ backupDevicesData: sortedDevicesData, devices: filteredDevicesData });
    ...
    deviceTypeFilter = (filters) => {
        ...
        return this.setState({ deviceTypeFilters: filters, devices: this.filterDevices(filters, backupDevicesData) });
    }

    onAddDevice = async (val) => {
        const { system_name, type, hdd_capacity } = val;
        const response = await deviceService.createDevice({ system_name, type, hdd_capacity });
    ...
    render() {
        const { isAddDeviceModalOpened, deviceTypeFilters, sortByType } = this.state;
        return (
                    ...
                    <div className='device-filter' >
                        <div className='device-filter-dropdown'>
                            <div className='device-type-dropdown'>
                                <Select defaultValue={deviceTypeFilters} placeholder="Select device filters" mode="multiple" style={{ minWidth: 200 }} onChange={(filters) => {
                            ...
                            <div className="sort-by-dropdown">
                                <Select defaultValue={sortByType} style={{ width: 150 }}
                                    onChange={(val) => {
                        ...
                        <div className='add-device'>
                            <Button type="primary" onClick={() => this.setState({ isAddDeviceModalOpened: true })}>Add Device</Button>
                        </div>
                    </div>
                    <div>
                        {this.state.devices.map(ele => <Device system_name={ele.system_name} type={ele.type} hdd_capacity={ele.hdd_capacity} key={ele.id} id={ele.id} onDeviceUpdated={() => this.fetchDevices()} />) }
                    </div>
                </div >
                {isAddDeviceModalOpened && <Modal isAddDevice isVisible={(val) => this.setState({ isAddDeviceModalOpened: val })} onAddEdit={(val) => this.onAddDevice(val)} />}
            </div >
        );
    }
}
